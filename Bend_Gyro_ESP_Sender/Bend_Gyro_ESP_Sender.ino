 ...
 *
 * 【操作ロジック】
 *   throttle = 曲げ量（0〜100、初速0・曲げるほど加速）
 *   steer    = ジャイロZ軸回転速度（-100〜+100、回した分だけ）
 *   left  = throttle - max(0,  steer)   … 右に回すと左輪が減速
 *   right = throttle - max(0, -steer)   … 左に回すと右輪が減速
 *
 * 【要再校正】
 *   BEND_MAX_DIFF … 旧SPRESENSE回路(1.8V)から3.3V回路に変わったため、
 *                    実測しながら調整すること（シリアルにraw diffを表示）
...
int       BEND_MAX_DIFF = 260;   // ★要再校正（3.3V回路での実測値に合わせる）
int flatValue = 0;
...
int readSteer() {
  ...
  if (readGyroRaw(gyroAddr, xr, yr, zr)) {
    ...
    return constrain((int)map((long)(gz * 100), (long)(-MAX_DPS * 100), (long)(MAX_DPS * 100), -100, 100), -100, 100);
  } else {
    gyroFailCount++;
...
void loop() {
  ...
  Serial.print("bend raw="); Serial.print(raw);
